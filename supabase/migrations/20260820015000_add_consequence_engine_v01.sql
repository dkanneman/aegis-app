create table if not exists public.consequences (
  id uuid primary key default gen_random_uuid(),
  household_id uuid not null references public.households(id) on delete cascade,
  consequence_type text not null check (consequence_type in ('missing_transport','missing_required_adult','person_conflict','driver_conflict')),
  severity text not null default 'needs_attention' check (severity in ('info','needs_attention','urgent')),
  status text not null default 'open' check (status in ('open','resolved','dismissed')),
  title text not null,
  summary text not null,
  affected_member_id uuid references public.household_members(id) on delete set null,
...
create index if not exists consequences_household_status_idx on public.consequences(household_id,status,severity,last_seen_at desc);
create index if not exists consequences_event_idx on public.consequences(event_id);
...
begin
  -- 1. Missing transportation for active transport events.
  for r in
    select e.*
  ...
  loop
    ...
    values (p_household_id,'missing_transport','needs_attention','open','Ride needed',r.title || ' still needs transportation.',r.id,v_fingerprint,now(),null,jsonb_build_object('starts_at',r.starts_at,'person_slug',r.person_slug))
  ...
  loop
    ...
    v_seen := array_append(v_seen, v_fingerprint);
    insert into public.consequences(household_id,consequence_type,severity,status,title,summary,event_id,fingerprint,last_seen_at,resolved_at,metadata)
    values (p_household_id,'missing_required_adult','needs_attention','open','Adult needed',r.title || ' requires an adult, but no adult is assigned.',r.id,v_fingerprint,now(),null,jsonb_build_object('starts_at',r.starts_at,'requirement',r.adult_requirement_label))
    on conflict (household_id,fingerprint) do update
      set status='open', title=excluded.title, summary=excluded.summary, last_seen_at=now(), resolved_at=null, metadata=excluded.metadata;
  end loop;

  -- 3. Same family member double-booked. Use a 60-minute default duration when ends_at is missing.
  for r in
  ...
  -- 4. Assigned driver has their own overlapping event.
  for r in
...
create or replace function public.trigger_recompute_consequences()
returns trigger
language plpgsql
set search_path = public, pg_temp
as $$
begin
  perform public.recompute_household_consequences(coalesce(new.household_id,old.household_id));
  return coalesce(new,old);
end;
$$;

drop trigger if exists trg_recompute_consequences_on_event on public.events;
create trigger trg_recompute_consequences_on_event
after insert or update or delete on public.events
for each row execute function public.trigger_recompute_consequences();

-- Seed current consequence state without fabricating historical findings.
do $$
declare h uuid; begin
  for h in select id from public.households loop
    perform public.recompute_household_consequences(h);
  end loop;
end $$;
