...
async function memberState(member:any,targetSlug:string){
  ...
  const [events,appointments,tasks,profiles,schoolChanges]=await Promise.all([
  ...
  return {member:target,events:memberEvents,tasks,school:profiles[0]?{profile:profiles[0],upcoming_changes:schoolChanges}:null}
}
